	...
	for _, claimResource := range pod.Status.ResourceClaimStatuses {
		...
		for _, result := range resourceClaim.Status.Allocation.Devices.Results {
			...
			if info.ResourceName == "" {
				err := fmt.Errorf("device %s missing required attribute %s (must match NAD k8s.v1.cni.cncf.io/resourceName)", result.Device, multusResourceNameAttr)
				logging.Errorf("GetPodResourceMap: %v", err)
				return err
			}
...
func (d *draClient) getDeviceInfo(ctx context.Context, result resourcev1api.DeviceRequestAllocationResult) (*deviceInfo, error) {
	...
	if !ok {
		...
		if len(matchingSlices) == 0 {
			err := fmt.Errorf("no resource slice found for driver/pool %s", key)
	...
	for _, resourceSlice := range resourceSlices {
		...
		for _, device := range resourceSlice.Spec.Devices {
			...
			if devIDAttr.StringValue == nil {
				logging.Errorf("getDeviceInfo: device %s has nil StringValue for %s attribute", device.Name, multusDeviceIDAttr)
				continue
			}
	...
	err := fmt.Errorf("device %s not found for claim resource %s/%s in any matching resource slice", result.Device, result.Driver, result.Pool)
	logging.Errorf("getDeviceInfo: %v", err)
	return nil, err
}
